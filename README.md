...
Aynı kişiye daha önce oluşmuş/reddedilmiş isteği tekrar gönderebilmek için ardından şunu çalıştır:

```text
supabase/manual/12_friend_request_resend_policy.sql
```

Eski/yarim kalmis `accepted` arkadaslik istegi kayitlarini tekrar bekleyen istege cevirebilmek icin son olarak sunu calistir:

```text
supabase/manual/13_friend_request_repair_policy.sql
```
