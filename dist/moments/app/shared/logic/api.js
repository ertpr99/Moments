...
export async function markMomentViewed(state, momentId) {
  const supabase = await getSupabase();
  const moment = state.moments.find((item) => item.id === momentId);
  if (!moment || (moment.open_count || 0) >= 2) return;
...
async function loadMoments(state) {
  ...
  const { data, error } = await supabase
    .from("moments")
    .select("id,sender_id,receiver_id,storage_path,created_at,expires_at,opened_at,open_count,sender:profiles!moments_sender_id_fkey(id,name,surname,username,profile_photo_url)")
    .eq("receiver_id", state.session.user.id)
    .lt("open_count", 2)
    .gt("expires_at", now)
