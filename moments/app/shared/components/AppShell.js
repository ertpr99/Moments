  ...
  const render = () => {
    ...
    root.innerHTML = `
      <section class="moments-app ${state.platform}">
        ${state.session && state.screen !== "moment" ? statusBar(state) : ""}
        <main class="screen-body">${screenContent(state)}</main>
        ${state.session && state.screen !== "moment" ? bottomNav(state) : ""}
    ...
    <nav class="bottom-nav">
      ${screens.map((screen) => `
        <button class="${state.screen === screen ? "active" : ""}" data-screen="${screen}" title="${tr[screen]}">
          <span>${navIcon(screen)}${navBadge(state, screen)}</span>
...
function navBadge(state, screen) {
  const count = screen === "home" ? state.incomingRequests.length : screen === "chat" ? chatBadgeCount(state) : 0;
  return count > 0 ? `<em class="nav-badge">+${count}</em>` : "";
}

function chatBadgeCount(state) {
  const unreadMessages = state.messages.filter((message) => message.sender_id !== state.session?.user?.id && !state.readMessageIds.includes(message.id)).length;
  const unopenedMoments = state.moments.filter((moment) => (moment.open_count || 0) === 0).length;
  return unreadMessages + unopenedMoments;
}
...
function screenContent(state) {
  ...
  if (state.screen === "moment") return momentViewerScreen(state);
...
function momentViewerScreen(state) {
  const moment = state.activeMomentQueue[state.activeMomentIndex];
  if (!moment) return emptyState("Moment bulunamadı.");
  const isVideo = String(moment.storage_path || "").toLowerCase().endsWith(".webm");
  return `
    <section class="moment-viewer-screen">
      <button class="moment-media-button" data-action="advance-moment" title="Sonraki Moment">
        ${isVideo
          ? `<video src="${escapeAttr(moment.signedUrl)}" autoplay playsinline controls></video>`
          : `<img src="${escapeAttr(moment.signedUrl)}" alt="Gelen Moment" />`}
      </button>
      <div class="moment-progress">
        ${state.activeMomentQueue.map((_, index) => `<span class="${index <= state.activeMomentIndex ? "active" : ""}"></span>`).join("")}
      </div>
    </section>
  `;
}
...
function chatListCard(state, person) {
  ...
  const hasMoment = state.moments.some((moment) => moment.sender_id === person.id && (moment.open_count || 0) === 0);
  const hasMessage = messages.some((message) => message.sender_id !== state.session.user.id && !state.readMessageIds.includes(message.id));
  return `
...
async function handleAction(state, action, root) {
  ...
  try {
    if (action.type === "screen") {
      ...
      if (action.screen !== "moment") {
        state.activeMomentQueue = [];
        state.activeMomentIndex = 0;
      }
      state.messageMenuId = "";
    }
    ...
    if (action.type === "advance-moment") await advanceMoment(state);
...
async function openChatAction(state, friendId) {
  ...
  const moments = state.moments
    .filter((moment) => moment.sender_id === friendId)
    .sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
  if (moments.length) {
    await startMomentQueue(state, moments);
    return;
  }
  markChatRead(state, friendId);
  state.screen = "chat";
}

async function startMomentQueue(state, moments) {
  const queue = [];
  for (const moment of moments) {
    const signedUrl = await openMoment(state, moment.id);
    if (signedUrl) queue.push({ ...moment, signedUrl });
  }
  if (!queue.length) {
    state.screen = "chat";
    return;
  }
  state.activeMomentQueue = queue;
  state.activeMomentIndex = 0;
  state.screen = "moment";
}

async function advanceMoment(state) {
  const moment = state.activeMomentQueue[state.activeMomentIndex];
  if (moment) await markMomentViewed(state, moment.id);
  if (state.activeMomentIndex + 1 < state.activeMomentQueue.length) {
...
function markChatRead(state, friendId) {
  const chat = chatForFriend(state, friendId);
  if (!chat) return;
  const next = new Set(state.readMessageIds);
  state.messages
    .filter((message) => message.chat_id === chat.id && message.sender_id !== state.session.user.id)
    .forEach((message) => next.add(message.id));
  state.readMessageIds = [...next];
  localStorage.setItem("moments-read-message-ids", JSON.stringify(state.readMessageIds));
}
...
async function sendCapturedMoment(state) {
  ...
  if (state.moments.some((moment) => moment.sender_id === state.activePersonId && (moment.open_count || 0) === 0)) {
