    ...
    >
      {screen === "friends" && (
        <FriendsScreen
          ...
          onCancelRequest={(requestId) => run(async () => {
      ...
      {profileView && <ProfileModal person={profileView} isFriend={friends.some((friend) => friend.id === profileView.id)} onClose={() => setProfileView(null)} onRequest={(id) => run(async () => { await sendFriendRequest(session.user.id, id); await refreshRequests(); })} onChat={(person) => { setProfileView(null); setActiveChatId(person.id); setScreen("chat"); }} />}
