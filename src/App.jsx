...
function Shell({ children, notice, title = "Moments", theme, setTheme, compact = false, hideChrome = false, action, onTouchStart, onTouchEnd }) {
  return (
...
function CameraScreen({ friends, incomingMoments, capturedMoment, setCapturedMoment, mode, setMode, selectedReceivers, setSelectedReceivers, hideMomentWarning, setHideMomentWarning, showWarning, setShowWarning, onSend }) {
  ...
  return (
    <section className="camera-full" onDoubleClick={() => setFacingMode(facingMode === "environment" ? "user" : "environment")} onClick={(event) => {
      ...
      <div className="zoom-row">{lenses.map((lens) => <button key={lens} className={zoom === lens ? "active" : ""} onClick={(event) => { event.stopPropagation(); setZoom(lens); }}>{lens}x</button>)}</div>
      <button className="shutter" onClick={(event) => { event.stopPropagation(); capture(); }} />
    </section>
  );
}
...
function ProfileEditor({ profile, onClose, onSave, onPhotoUpload, onUsernameCheck }) {
  ...
  return (
    <div className="full-panel">
      ...
      <input ref={fileRef} className="visually-hidden-file" type="file" accept="image/*" onChange={(event) => event.target.files?.[0] && onPhotoUpload(event.target.files[0])} />
      <div className="segmented">
