...
export default function App() {
  ...
  const [momentReceipts, setMomentReceipts] = useState([]);
  ...
  async function refreshAll(user = session?.user) {
    ...
    const [nextFriends, nextRequests, nextChats, nextMoments, nextReceipts, nextDiscover] = await Promise.all([
      ...
      loadMomentReceipts(user.id).catch(() => []),
      loadDiscover().catch(() => [])
    ]);
    ...
    setMomentReceipts(nextReceipts);
  ...
  async function requestFriendship(personId) {
    const request = await sendFriendRequest(session.user.id, personId);
    setRequests((current) => {
      const outgoing = current.outgoing || [];
      const nextRequest = {
        ...request,
        sender_id: request.sender_id || session.user.id,
        receiver_id: request.receiver_id || personId
      };
      const exists = outgoing.some((item) => item.receiver_id === personId);
      return {
        ...current,
        outgoing: exists
          ? outgoing.map((item) => item.receiver_id === personId ? { ...item, ...nextRequest } : item)
          : [...outgoing, nextRequest]
      };
    });
  ...
  async function cancelOutgoingRequest(requestId) {
    ...
    setRequests((current) => ({
      ...current,
      outgoing: (current.outgoing || []).filter((request) => request.id !== requestId)
    }));
    await refreshRequests();
  }
  ...
  async function refreshMoments(userId = session?.user?.id) {
    if (!userId) return;
    const [nextIncoming, nextReceipts] = await Promise.all([
      loadIncomingMoments(userId),
      loadMomentReceipts(userId)
    ]);
    setIncomingMoments(nextIncoming);
    setMomentReceipts(nextReceipts);
  }
    ...
    >
      {screen === "friends" && (
        <FriendsScreen
          ...
          onRequest={(personId) => run(async () => requestFriendship(personId))}
          onCancelRequest={(requestId) => run(async () => cancelOutgoingRequest(requestId))}
      ...
      {screen === "chat" && !activeChatId && (
        <ChatList
          ...
          onOpen={(friendId, replay = false) => run(async () => {
            const moment = incomingMoments.find((item) => item.sender_id === friendId);
            setActiveChatId(friendId);
            if (moment && (replay || (moment.open_count || 0) === 0)) {
      ...
      {screen === "chat" && activeChatId && !activeMoment && (
        <ChatDetail
          ...
          messages={messages}
          momentReceipts={momentReceipts.filter((moment) => moment.receiver_id === activeChatId)}
      ...
      {profileView && <ProfileModal person={profileView} isFriend={friends.some((friend) => friend.id === profileView.id)} outgoing={requests.outgoing} onClose={() => setProfileView(null)} onRequest={(id) => run(async () => requestFriendship(id))} onCancelRequest={(requestId) => run(async () => cancelOutgoingRequest(requestId))} onChat={(person) => { setProfileView(null); setActiveChatId(person.id); setScreen("chat"); }} />}
      ...
      {!activeMoment && <nav className="bottom-nav">
        {tabs.map((tab) => (
...
function FriendsScreen({ friends, requests, searchQuery, setSearchQuery, searchResults, onSearch, onRequest, onCancelRequest, onRespond, onProfile, onChat }) {
        ...
        <div className="panel-list">
          {searchResults.length ? searchResults.map((person) => (
            <article className="person-card" key={person.id}>
              ...
              <button className="person-main person-profile-button" onClick={() => onProfile(person)}>
                <PersonText person={person} />
              </button>
              <RequestAction person={person} outgoing={requests.outgoing} onRequest={onRequest} onCancelRequest={onCancelRequest} />
            </article>
...
function RequestAction({ person, outgoing, onRequest, onCancelRequest }) {
  const pending = outgoing.find((request) => request.receiver_id === person.id);
  if (pending) {
    return (
      <div className="request-sent-actions">
        <span>Arkadaşlık gönderildi</span>
        <button className="small-action muted" onClick={() => onCancelRequest(pending.id)}>İsteği iptal et</button>
      </div>
    );
  }
  return <button className="small-action" onClick={() => onRequest(person.id)}>Arkadaş ekle</button>;
}
...
function ChatList({ friends, chats, messages, incomingMoments, userId, query, setQuery, onOpen }) {
  ...
  function startOpenPress(friendId, canReplay) {
    clearTimeout(longPressRef.current?.timer);
    if (!canReplay) {
      longPressRef.current = { friendId, used: false };
      return;
    }
    longPressRef.current = {
      ...
      used: false,
      timer: setTimeout(() => {
        longPressRef.current = { friendId, used: true };
        if (canReplay) onOpen(friendId, true);
      }, 520)
    };
  }

  function finishOpenPress(friendId) {
    const state = longPressRef.current;
    if (!state) return;
    clearTimeout(state?.timer);
    longPressRef.current = null;
    if (!state?.used) onOpen(friendId, false);
  }

  function cancelOpenPress() {
    clearTimeout(longPressRef.current?.timer);
    longPressRef.current = null;
  }
  ...
  return (
    <section className="stack">
      ...
      {filtered.length ? filtered.map((friend) => {
        ...
        const moment = incomingMoments.find((item) => item.sender_id === friend.id);
        const hasMoment = Boolean(moment);
        const canReplayMoment = hasMoment && (moment.open_count || 0) > 0;
        return (
          <button
            className="chat-row"
            key={friend.id}
            onMouseDown={() => startOpenPress(friend.id, canReplayMoment)}
            onMouseUp={() => finishOpenPress(friend.id)}
            onMouseLeave={cancelOpenPress}
            onTouchStart={() => startOpenPress(friend.id, canReplayMoment)}
            onTouchEnd={() => finishOpenPress(friend.id)}
            onTouchCancel={cancelOpenPress}
            onKeyDown={(event) => {
              if (event.key === "Enter" || event.key === " ") onOpen(friend.id, false);
            }}
...
function ChatDetail({ userId, friend, chat, messages, momentReceipts = [], menuId, setMenuId, onBack, onSend, onDeleteEveryone, onDeleteSelf }) {
  const [body, setBody] = useState("");
  const visibleMessages = chat ? messages.filter((message) => message.chat_id === chat.id) : [];
  const openedTwiceMoments = momentReceipts.filter((moment) => (moment.open_count || 0) >= 2);
  return (
    <section className="stack chat-detail">
      ...
      <div className="chat-list">
        {openedTwiceMoments.map((moment) => (
          <article className="moment-receipt mine" key={moment.id}>
            <span>Gönderdiğin Moment</span>
            <small>2 defa açıldı</small>
          </article>
        ))}
...
function CameraScreen({ friends, incomingMoments, capturedMoment, setCapturedMoment, mode, setMode, selectedReceivers, setSelectedReceivers, hideMomentWarning, setHideMomentWarning, showWarning, setShowWarning, onSend }) {
  ...
  const [zoomRange, setZoomRange] = useState({ min: 0.5, max: 10 });
  ...
  function updateZoomRange(stream = streamRef.current) {
  ...
  function setSafeZoom(nextZoom) {
    setZoom(Math.min(zoomRange.max, Math.max(zoomRange.min, Number(nextZoom.toFixed(2)))));
  }

  function chooseZoom(lens) {
    if (lens === 0.5) {
      const wide = videoDevices.find((device) => /ultra|wide|0\.5|back|rear|environment/i.test(device.label));
      if (wide && wide.deviceId !== selectedDeviceId) {
        setSelectedDeviceId(wide.deviceId);
  ...
  function capture() {
    ...
    const cropScale = Math.max(1, zoom);
  ...
  function startVideoRecording() {
    ...
    if (!stream || !window.MediaRecorder) {
      setCameraError("Video kaydı bu tarayıcıda desteklenmiyor.");
      return;
    }
    ...
    recorder.onstop = () => {
      ...
      setCapturedMoment({ blob, dataUrl: url, kind: "video", mimeType });
      setRecording(false);
      setMode("preview");
    };
    recorder.start(200);
  ...
  function save() {
    ...
    link.download = capturedMoment.kind === "video" ? "moment.webm" : "moment.png";
    link.click();
  }
  ...
  if (mode === "preview" && capturedMoment) {
    return (
      <section className="moment-compose">
        {capturedMoment.kind === "video"
          ? <video src={capturedMoment.dataUrl} autoPlay playsInline controls loop />
    ...
    <section
      ...
      onTouchMove={(event) => {
        if (event.touches.length === 2 && pinchRef.current) {
          event.preventDefault();
          const distance = touchDistance(event.touches);
          setSafeZoom(pinchRef.current.zoom * (distance / pinchRef.current.distance));
        }
      }}
      onTouchEnd={() => {
      ...
      <video ref={videoRef} autoPlay playsInline muted className={facingMode === "user" ? "front-camera-preview" : ""} style={{ transform: `${facingMode === "user" ? "scaleX(-1) " : ""}scale(${Math.max(1, zoom)})` }} />
      {cameraError && <div className="camera-fallback">{cameraError}</div>}
      {focus && <span className="focus-ring" style={{ left: focus.x, top: focus.y }} />}
      {recording && <div className="recording-pill"><span /> Kaydediliyor</div>}
      <button className="flip-camera" onClick={(event) => { event.stopPropagation(); flipCamera(); }}>⇄</button>
      <div className="zoom-row">{lenses.map((lens) => <button key={lens} className={Math.abs(zoom - lens) < 0.05 ? "active" : ""} onClick={(event) => { event.stopPropagation(); chooseZoom(lens); }}>{lens}x</button>)}</div>
      <button
        className="shutter"
        onPointerDown={(event) => {
          ...
          recordTimerRef.current = setTimeout(() => {
            if (shutterDragRef.current) {
              shutterDragRef.current.recording = true;
              startVideoRecording();
            }
          }, 360);
...
function MomentViewer({ moment, onClose }) {
  const isVideo = moment.storage_path?.toLowerCase().endsWith(".webm");
  return (
    <div className="moment-overlay">
      <div className="moment-viewer">
        {isVideo
          ? <video src={moment.signedUrl} autoPlay playsInline controls />
...
function ProfileModal({ person, isFriend, outgoing = [], onClose, onRequest, onCancelRequest, onChat }) {
  return (
    <div className="full-panel profile-modal">
      ...
      {isFriend ? <button className="small-action" onClick={() => onChat(person)}>Mesaj Gönder</button> : <RequestAction person={person} outgoing={outgoing} onRequest={onRequest} onCancelRequest={onCancelRequest} />}
    </div>
  );
}
