    ...
    <Shell theme={theme} setTheme={setTheme} notice={notice} title={activeFriend && screen === "chat" ? fullName(activeFriend) : tabs.find((tab) => tab.id === screen)?.label || "Moments"}>
      ...
      {screen === "settings" && (
        <SettingsScreen
          ...
          onPhotoUpload={(file) => run(async () => {
            const nextProfile = await updateProfilePhoto(session.user.id, file);
            setProfile(nextProfile);
...
function Shell({ children, notice, title = "Moments", theme, setTheme, compact = false }) {
  return (
    <main className="app-shell">
      <section className={compact ? "phone-app auth-frame" : "phone-app"}>
  ...
  return (
    <section className="auth-screen">
      <div className="auth-intro">
        <h2>Moments'a hoş geldin.</h2>
      </div>
      <div className="segmented auth-toggle" role="tablist" aria-label="Giriş ve kayıt">
      ...
      <form onSubmit={(event) => { event.preventDefault(); onSubmit(form); }} className="form auth-form">
        {signup && (
          <>
            <input autoComplete="given-name" placeholder="Ad" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required />
            <input autoComplete="family-name" placeholder="Soyad" value={form.surname} onChange={(event) => setForm({ ...form, surname: event.target.value })} required />
            <input autoComplete="username" placeholder="Kullanıcı Adı" value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} required />
          </>
        )}
        <input autoComplete="email" type="email" placeholder="E-posta" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} required />
        <input autoComplete={signup ? "new-password" : "current-password"} type="password" placeholder="Şifre" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} required minLength={6} />
...
function SettingsScreen({ theme, setTheme, onSignOut, onPhotoUpload }) {
  ...
  return (
    <section className="stack">
      ...
      <article className="settings-row">
        <div><strong>Profil fotoğrafı</strong><small>Hesabını oluşturduktan sonra dilediğin zaman güncelle.</small></div>
        <button className="small-action" onClick={() => fileRef.current?.click()}>Fotoğraf yükle</button>
        <input
          ref={fileRef}
