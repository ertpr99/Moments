...
export async function signUp(form) {
  ...
  if (error) throw error;
  if (!data.session) throw new Error("Kayıt oluşturuldu. Email doğrulaması açıksa emailini onaylayıp giriş yap.");

  let profilePhotoUrl = null;
  if (form.profilePhoto) {
    profilePhotoUrl = await uploadProfilePhoto(data.user.id, form.profilePhoto);
  }
