...
export async function updateProfileDetails(profile, form) {
  ...
  if (currentName !== nextName) {
    if (!canChangeAfter(profile.name_changed_at, 30)) throw new Error("Ad bilgisi 30 günde bir değiştirilebilir.");
    payload.name_changed_at = now;
  }
  if (profile.username !== payload.username) {
    if (!canChangeAfter(profile.username_changed_at, 14)) throw new Error("Kullanıcı adı 14 günde bir değiştirilebilir.");
    payload.username_changed_at = now;
  }
  if ((profile.account_type || "personal") !== accountType) {
    if (!canChangeAfter(profile.account_type_changed_at, 30)) throw new Error("Hesap türü 30 günde bir değiştirilebilir.");
...
export async function loadFriends(userId) {
  const { data, error } = await supabase
    ...
    .eq("user_id", userId);
  if (error) throw error;
  return (data || []).map((row) => row.friend).filter((friend) => friend && !isFrozen(friend) && !friend.deleted_at);
}
...
export async function loadRequests(userId) {
  ...
  return {
    incoming: (incoming || []).filter((request) => request.sender && !isFrozen(request.sender) && !request.sender.deleted_at),
    outgoing: outgoing || []
  };
}
...
export async function searchUsers(userId, query, friendIds = []) {
  ...
  return (data || []).filter((person) => !friends.has(person.id) && !isFrozen(person) && isProfileCompatible(person));
}
...
export async function loadIncomingMoments(userId) {
  ...
  if (error) throw error;
  return (data || []).filter((moment) => moment.sender && !isFrozen(moment.sender) && !moment.sender.deleted_at);
}
...
export async function loadDiscover() {
  ...
  const { data, error } = await request;
  if (error) throw error;
  return (data || []).filter((person) => !isFrozen(person) && isProfileCompatible(person));
}
