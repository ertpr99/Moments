...
-- 04_functions_triggers.sql sonrasinda calistirilir. RLS kurallarini kurar.
...
create policy "friend requests receiver update" on public.friend_requests for update using (auth.uid() = receiver_id) with check (auth.uid() = receiver_id);
...
create policy "chats updated by members" on public.chats for update using (auth.uid() in (member_a, member_b)) with check (auth.uid() in (member_a, member_b));

drop policy if exists "messages visible to chat members" on public.messages;
create policy "messages visible to chat members" on public.messages for select using (exists (select 1 from public.chats where chats.id = messages.chat_id and auth.uid() in (member_a, member_b)));
drop policy if exists "messages inserted by chat members" on public.messages;
create policy "messages inserted by chat members" on public.messages for insert with check (auth.uid() = sender_id and exists (select 1 from public.chats where chats.id = messages.chat_id and auth.uid() in (member_a, member_b)));
drop policy if exists "messages updated by sender" on public.messages;
create policy "messages updated by sender" on public.messages for update using (auth.uid() = sender_id) with check (auth.uid() = sender_id);
...
create policy "moments receiver update" on public.moments for update using (auth.uid() = receiver_id) with check (auth.uid() = receiver_id);
...
create policy "moment views updated by viewer" on public.moment_views for update using (auth.uid() = viewer_id) with check (auth.uid() = viewer_id);
